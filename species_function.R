...
clean_taxon<-function(data=ISSPECIESCODES, taxa=taxa)
{
  ...
  # Level 1: Scientific Names Match Reference Table
    ...
    mutate(SCIENTIFIC=str_replace(SCIENTIFIC, c("[[:punct:]]","\t"),""))%>%    mutate(SCIENTIFIC=str_replace(SCIENTIFIC, "  "," "))%>%
    left_join(taxa)%>%
    filter(!is.na(Genus))
  ...
  taxa_order<-taxa%>%
    ...
    mutate(SCIENTIFIC=str_replace(SCIENTIFIC, c("[[:punct:]]","\t"),""))%>%
